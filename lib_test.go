...
func TestConvolveSame(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				xs: []float64{0, 1, 2, 3, 4, 5},
				ys: []float64{0.2, 0.8, 0.6, 0.4},
			},
			want: []float64{0, 0.2, 1.2, 2.2},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ConvolveSame(tt.args.xs, tt.args.ys); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("ConvolveSame() = %v, want %v", got, tt.want)
			}
		})
	}
}