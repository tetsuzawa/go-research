...
y_k = \sum\emph{{i=1}^{}K w_i^{}* x}{k-i+1}
...
\sigma\emph{d^{}2 := E{[}d_k d_k^{}H{]} 、 \bm{r}}{xd} :=
\bm{x}\emph{k d_k^{}H 、 \bm{r}}{dx} := d_k \bm{x}_k^{}H 、
...
J = \sum_{k=1}^{}{L_s} \textbar{}\epsilon\emph{k\textbar^{}2 =
...
\bm{w}\emph{k &= \bm{w}}{k-1} + \mu (E{[}\bm{x}_k d_k^{}*{]} -
E{[}\bm{x}\emph{k \bm{x}}^{}H{]} \bm{w}\emph{{k-1}) &=
\bm{w}}{k-1} + \mu E{[}\bm{x}_k (d_k^{}* - \bm{x}\emph{k^{}H
\bm{w}}{k-1}){]}
...
\bm{w}\emph{k = \bm{w}}{k-1} + \mu E{[}\bm{x}_k e_k^{}*{]}
...
\bm{x}_k := {[}\bm{x}\emph{{k}, \cdots, \bm{x}}{k-L_s+1}{]}^{}T
...
\hat{\bm{R}_k} := \sum_{i=1}^{}k \bm{x}\emph{i \bm{x}\emph{i^{}H =
\bm{X}}{1:k} \bm{X}}{1:k}^{}H
\end{equation}
...
\bm{w}_{k-1} + \bm{g}_k (d_k^{}* - \bm{x}\emph{k^{}H
\bm{w}}{k-1})
\end{equation}
...
\begin{equation}
\bm{w}\emph{k = \bm{w}}{k-1} + \gm{g}_k e_k^{}*
\end{equation}
