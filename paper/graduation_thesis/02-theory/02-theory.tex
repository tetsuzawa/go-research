...
\begin{figure}[H]
\centering
\includegraphics[width=13cm]{figures/block_jp.png}
...
一方、フィルタリング過程では、信号\(d_k\)が未知の場合について、観測値\(\bm{x}_k\)と学習済みのフィルタ係数\(\bm{w}\)から、信号の推定値\(\hat{d_k} (= y_k)\)を得る。学習過程で望みの応答\(d_k\)をどうやって与えるかは、応用に依存する\cite{signal_proccesing_for_array}。
...
\begin{figure}[H]
\centering
\includegraphics[width=13cm]{figures/anc.png}
...
自動等化器のブロック図を\equref{equ:automatic_equlizer_block}に示す。
