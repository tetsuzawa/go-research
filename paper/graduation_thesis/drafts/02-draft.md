...
![ウィナーフィルタのブロック図](figures/block_jp.png)
...
ここで、$\bm{d}_{1:k}$および$\bm{X}_{1:k}$は、次式で定義するブロックデータである。
...
![代表的な適応アルゴリズムの関係](figures/algo_table_clac.png)
...
![ドローンの駆動音に対するANCの想定ブロック図](figures/anc.png)
...
![ドローンの駆動音に対する自動等化器の想定ブロック図](figures/automatic_equalizer_block.png)
