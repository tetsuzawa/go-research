...
\item
  ...
  \item
    ...
    \end{itemize}
  \item
    \ref{の実験で用いるADFの設定を記述したJSONファイルを生成するプログラム

    \begin{itemize}
    \tightlist
    \item
      \ref{drone\_json\_generator}
    \end{itemize}
  \item
...
Pythonで記述された適応信号処理のライブラリとして\cite{https://matousc89.github.io/padasip/index.html\#padasip}{Padasip}が公開されている。Padasipは信号のフィルタリング、予測、復元、分類といった適応信号処理を簡易化するために設計されたライブラリであり、LMS・NLMS、AP、RLSをはじめとする主要なアルゴリズムが一通り実装されている。
