...
\begin{table}[H]
  ...
  & \begin{tabular}{c}2つのwavファイルを畳み込み \\（フーリエ変換を用いた高速版）\end{tabular}                          & 1.2.1-7  \\ \cline{2-3} 
                         ...
                         & \begin{tabular}{c}\ref{adf-practice}の実験で用いるADFの設定を記述した\\JSONファイルを生成\end{tabular}           & 1.2.1-10 \\ \cline{2-3} 
                         & \begin{tabular}{c}csvファイルのデータから指定したサンプルの平均二乗誤差（MSE）\\を計算し, csvファイルに新たな列として追記\end{tabular}  & 1.2.1-11 \\ \cline{2-3} 
